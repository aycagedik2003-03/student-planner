  ...
  return (
    <SafeAreaView style={st.root}>
      <View style={st.header}>
        ...
        <Text style={st.title}>{t('settings.changePassword')}</Text>
        <View style={{ width: 38 }} />
      </View>
          ...
          return (
            <View key={label}>
              ...
              <TextInput
                style={st.input}
                value={vals[i]}
                onChangeText={v => { setters[i](v); setError(''); }}
                placeholder="••••••••"
...
const st = StyleSheet.create({
  ...
  strength:   { backgroundColor: '#F9FAFB', borderRadius: 10, padding: 14, marginBottom: 16, gap: 4 },
  strengthItem:{ fontSize: 12, fontWeight: '600' },
  strengthOk:  { color: C.brand },
  strengthFail:{ color: C.error },
